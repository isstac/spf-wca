...
import wcanalysis.charting.*;
import wcanalysis.fitting.*;
import wcanalysis.heuristic.*;
 ...
 */
public class WorstCaseAnalyzerNew implements JPFShell {
  ...
  public WorstCaseAnalyzerNew(Config config) {
    this.logger = JPF.getLogger(WorstCaseAnalyzerNew.class.getName());
    this.config = config;
  ...
  private void getPolicy(Config jpfConf) {
    ...
    jpf.addListener(new WCPolicyGeneratorListener(jpfConf, jpf)); //weird instantiation...
  ...
  private DataCollection performAnalysis(Config jpfConf) {
    ...
    for(int inputSize = 0; inputSize <= maxInput; inputSize++) {//TODO: should maxInput be included?
      ...
      JPF jpf = new JPF(jpfConf);
      WCHeuristicListener heuristic = new WCHeuristicListener(jpfConf, jpf);
      jpf.addListener(heuristic); //weird instantiation...

      //explore guided by policy
      jpf.run();
      int wc = heuristic.getWCIntValue();
      dataCollection.addDatapoint(inputSize, wc);
    }
